...
class CompactBilinearPooling(torch.nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        fft1=torch.view_as_real(torch.fft.fft(x1.permute(0, 2, 3, 1).matmul(self.sketch_matrix1), 1))
        fft2=torch.view_as_real(torch.fft.fft(x2.permute(0, 2, 3, 1).matmul(self.sketch_matrix2), 1))
        ...
        cbp= torch.fft.irfft(torch.view_as_complex(fft_product), n=fft_product.shape[1], dim=1)
